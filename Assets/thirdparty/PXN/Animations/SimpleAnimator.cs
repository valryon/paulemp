...
public sealed class SimpleAnimator : MonoBehaviour
{
  ...
  public const float SLOW_ANIMATION_SPEED = 0.5f;
  ...
  void Update()
  {
    ...
    if (currentFrame >= currentAnimation.frames.Length)
    {
      if (currentAnimation.loop == false)
      {
        ...
        {
          AnimationCompleted(currentAnimation.name);
  ...
  public void Play(SimpleAnimation anim, bool reset)
  {
    ...
    if (currentAnimation.frames == null || currentAnimation.frames.Length == 0)
    {
  ...
  private SimpleAnimation[] Find(string animationName)
  {
    var anims = new List<SimpleAnimation>();

    foreach (var clip in clips)
    {
      if (animationName.ToLower() == clip.name.ToLower())
      {
        anims.Add(clip);
      }
    }
